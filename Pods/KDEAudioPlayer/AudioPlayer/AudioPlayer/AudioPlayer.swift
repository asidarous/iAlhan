...
open class AudioPlayer: NSObject {
    ...
    public override init() {
        state = .buffering
        super.init()
        observe(ReachabilityChangedNotification.rawValue, selector: "reachabilityStatusChanged:", object: reachability)
        do{
    ...
    fileprivate let reachability = Reachability.init()// .reachabilityForInternetConnection()
    ...
    open fileprivate(set) var currentItem: AudioItem? {
        didSet {
            ...
            if let currentItem = currentItem {
                ...
                if reachability!.isReachable || URLInfo.URL.isFileURL || URLInfo.URL.scheme == "ipod-library" {
                    state = .buffering
                }
                else {
    ...
    open func playItems(_ items: [AudioItem], startAtIndex index: Int = 0) {
        if items.count > 0 {
            var idx = 0
            enqueuedItems = items.map { (position: idx + 1 , item: $0) }
            adaptQueueToPlayerMode()
    ...
    open func addItemsToQueue(_ items: [AudioItem]) {
        if currentItem != nil {
            var idx = 0
            enqueuedItems = (enqueuedItems ?? []) + items.map { (position: idx + 1, item: $0) }
            adaptQueueToPlayerMode()
        }
        else {
            playItems(items)
    ...
    @objc fileprivate func audioSessionGotInterrupted(_ note: Notification) {
        if let typeInt = note.userInfo?[AVAudioSessionInterruptionTypeKey] as? UInt, let type = AVAudioSessionInterruptionType(rawValue: typeInt) {
